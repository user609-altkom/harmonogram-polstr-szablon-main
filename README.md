...
Repozytorium zawiera aplikację Next.js (App Router, TypeScript, Tailwind) z czystą domeną obliczeniową, seriami wskaźników, API harmonogramu, ekranem kalkulatora i testami Vitest, a także spec-kit, workflow GitHub Actions, reguły review dla Copilota i skrypty rutyny review przez Copilot CLI.
...
- `app/api/harmonogram/route.ts`: `GET /api/harmonogram`, parsuje parametry, dobiera serię POLSTR/WIBOR, woła domenę i zwraca JSON.
- `app/page.tsx`: ekran kalkulatora z formularzem, podsumowaniem, tabelą rat i eksportem CSV.
...
6. Uruchom aplikację lokalnie i sprawdź w przeglądarce http://localhost:3000 oraz endpoint `/api/harmonogram` z parametrami. Zatrzymaj serwer klawiszami Ctrl+C:
